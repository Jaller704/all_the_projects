...
Integer _subtract(Integer self, Integer i) {
    ...
    if(self->_val > -1 && i->_val > -1){    //positives
        valid = i->_val < self->_val ? INT_MIN < self->_val - i->_val
                                     : true;
    } else if(self->_val > -1 && i->_val < 0){  //self positive, i negative, should always be a positive result
        valid = 0 < self->_val - i->_val && self->_val - i->_val <= INT_MAX;
    } else if(i->_val > -1 && self->_val < 0){  //self negative, i positive, should never be a positive result
        valid = 0 > self->_val - i->_val && self->_val - i->_val >= INT_MIN;
    } else {    // both negative, could be negative or positve but should never underflow or overflow
        valid = true;
    }
...
/* _divide: implemented, do NOT change */
Integer _divide(Integer self, Integer i) {
    ...
    if (i->_val == 0){
       valid = false;
    } else if (self->_val == INT_MIN && i->_val == -1) {
        valid = false;
    } else {
        valid = true;
    }
...
/* _modulo: implemented, do NOT change */
