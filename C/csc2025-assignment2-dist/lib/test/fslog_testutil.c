...
void print_fslog(struct fsloginf *fsloginf, struct fslogrec *fslog) {
    ...
    printf("%s\n", DELIM);
    while (entries) {
        printf("[%3d] opcode: %0#4o | result: %4d | fd_nr: %2d | mode: %#o | uid: %d | gid: %d | sz: %d | %s | %s", 
            i, fslog[i].opcode, fslog[i].result, fslog[i].fd_nr, 
            fslog[i].v_mode, fslog[i].v_uid, fslog[i].v_gid,
            fslog[i].v_size, fslog[i].path, ctime(&fslog[i].timestamp));
