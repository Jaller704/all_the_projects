...
/* Perform read(fd, buffer, nbytes) or write(fd, buffer, nbytes) call. */
  ...
  if (scratch(fp).io.io_nbytes == 0) {
    /* CSC2025 mod start */
    logfsop_nopath(fsopcode, OK, scratch(fp).file.fd_nr, UNKNOWN_V_MODE, UNKNOWN_V_UID, UNKNOWN_V_GID,
    UNKNOWN_V_SIZE);  
    /* CSC2025 mod end */
	  unlock_filp(f);
  ...
  /* CSC2025 mod start */
  if(r == OK){
    logfsop_nopath(fsopcode, r, scratch(fp).file.fd_nr, UNKNOWN_V_MODE, UNKNOWN_V_UID, UNKNOWN_V_GID,
                   UNKNOWN_V_SIZE);
  } else {
