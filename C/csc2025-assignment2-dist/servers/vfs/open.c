 ...
 */
  ...
  if (copy_name(vname_length, fullpath) != OK) {
	...
	if (fetch_name(vname, vname_length, fullpath) != OK) {
	    /* CSC2025 mod start */
	    logfserr_nopath(FSOP_CREAT, err_code);
	    /* CSC2025 mod end */
		return(err_code);
	}
  }
...
/* Common code from do_creat and do_open. */
  ...
  /* CSC2025 mod start */
  unsigned short fsopcode = FSOP_OPEN;  // distinguish open from create
  /* CS2025 mod end */
  ...
  /* CSC2025 mod start */
  /* log before unlock - need fd_nr if OK - see line 341 to get it */
  if (r == OK)
    logfsop(fsopcode, r, path, scratch(fp).file.fd_nr, vp->v_mode, vp->v_uid,
        vp->v_gid, vp->v_size);
  else
    logfserr(fsopcode, r, path);    
  /* CSC2025 mod end */
...
/* Perform the close(fd) system call. */
  ...
  logfsop_nopath(FSOP_CLOSE, OK, fd_nr, vp->v_mode, vp->v_uid, vp->v_gid,
    vp->v_size);  
  /* CSC2025 mod end */
