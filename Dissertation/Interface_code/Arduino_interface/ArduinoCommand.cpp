...
ArduinoCommand::ArduinoCommand(string raw_command) {
	...
	if (first_space == string::npos) {
		this->command_name = raw_command.substr(first_letter);
	}
	else {
		//Pull this name out of the string and set the command_name
		this->command_name = raw_command.substr(first_letter, (first_space - first_letter));
	}

	//Check if the command has parameters or not
	if (this->command_name == "CONFIG" || this->command_name.compare(0, 3, "GET") == 0) {
		stringstream ss;
		ss << command_name << "\r\n";
		...
		//Recombine the name and parameters to create a string formatted for the serial console
		stringstream ss;
		ss << command_name;
